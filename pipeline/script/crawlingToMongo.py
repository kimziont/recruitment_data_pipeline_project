...
try:
    ...
    # 카테고리 전환해서 다음 데이터 하나 뽑아내기까지 30초 정도 걸림
    ...
    # 마갑된 채용 공고 Status 변경하기 (In progress -> Closed)
    # 사실 이 방법은 더 많은 데이터를 수집한다는 점에서는 의미가 있지만,
    # 지금 이 구현이 제대로 된 방법은 아니다.
    # 왜냐하면 항상 크롤링이 100% 잘 수행된다는 보장이 없기 때문에, 중간에 크롤링에 실패하는 데이터가 생기고,
    # 그로인해 진행중(In progress)인 공고가 마치 마감(Closed)된 것처럼 표현되기 때문이다.
